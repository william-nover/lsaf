...
class Mock_test_ajax extends MY_Controller {
	public $data = array();
	...
	public function __construct()
	{
		...
		$this->load->model(array('web/Model_mock_exam'));
    }
	...
	public function SubmitRadio(){
		...
		if(!empty($q_id) && !empty($a_id) && !empty($groupid)){
			$rsExam	= $this->Model_mock_exam->getExam($q_id,$student_id,$groupid);
			$countExam 	= count($rsExam);
			
			if($countExam > 0){
				$this->Model_mock_exam->updateExam($q_id,$a_id,$student_id,$groupid);
			} else {
				$exam_id	= $this->Model_mock_exam->insertExam($q_id,$a_id,$student_id,$groupid);
			}
	...
	public function SubmitFinish(){
		...
		if(!empty($csfr) && !empty($student_id) && !empty($groupid) && !empty($type)){
			...
			$_SESSION['mock_test']['exam'][$groupid]['start'] = 3;
		} else {
	...
	function uploadEssay(){
		...
		$output_dir = PATH_ASSETS."/upload/mock/";
