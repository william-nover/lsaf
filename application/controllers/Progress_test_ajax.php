...
class Progress_test_ajax extends MY_Controller {
	public $data = array();
	...
	public function __construct()
	{
		...
		$this->load->model(array('web/Model_progress_exam'));
    }
	...
	public function SubmitRadio(){
		...
		if(!empty($q_id) && !empty($a_id) && !empty($groupid)){
			$rsExam	= $this->Model_progress_exam->getExam($q_id,$student_id,$groupid);
			$countExam 	= count($rsExam);
			
			if($countExam > 0){
				$this->Model_progress_exam->updateExam($q_id,$a_id,$student_id,$groupid);
			} else {
				$exam_id	= $this->Model_progress_exam->insertExam($q_id,$a_id,$student_id,$groupid);
			}
	...
	public function SubmitFinish(){
		...
		if(!empty($csfr) && !empty($student_id) && !empty($groupid) && !empty($type)){
			...
			for($a=0;$a<count($getAllExamQuestionAnswered);$a++){
				$question_id = $getAllExamQuestionAnswered[$a]['progress_exam_question_id'];
				$answer_id = $getAllExamQuestionAnswered[$a]['progress_exam_answer_id'];
				$checkAnswer = $this->Model_progress_exam->checkAnswer($question_id, $answer_id);
				
				if(count($checkAnswer) > 0){
					$answer_status = $checkAnswer[0]['progress_exam_answer_status'];
					if($answer_status == 1){
			...
			$_SESSION['progress_test']['exam'][$groupid]['start'] = 3;
		} else {
	...
	function uploadEssay(){
		...
		$output_dir = PATH_ASSETS."/upload/progress/";
