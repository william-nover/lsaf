...
class Discover_acca extends CI_Controller {
	public $arrMenu = array();
	public $data;
	public $privilege = array();
	public $section = 8; //get module group id from database
	public $module_id = 17; //get module id from database
	public $alias_category = "discover_acca";
	public $module = "Discover_acca";
...
//                die;
	...
	function view(){
		...
		$base_url		= BASE_URL_BACKEND."/discover_acca/view/";
	...
	public function doAdd(){
		...
		$tb = $_POST['tbSave'];
		if (!$tb) {
			redirect(BASE_URL_BACKEND."/discover_acca");
			exit();
		}
		...
		if (! count($pesan)==0 ) {
			...
			if ($countContent > 0 ) {
				...
				if ($countAlias > 0 ) {
					...
					if(!empty($content_id)) {
						$aliasid = $this->Model_alias->insertAlias($content_id,$this->alias_category,$alias,"discover_acca/detail/".$content_id);
					}
