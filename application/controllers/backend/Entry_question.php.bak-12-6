...
class Entry_question extends CI_Controller {
	...
	public function doAdd(){
		...
		if (! count($pesan)==0 ) {
			...
			if ($countQuestion > 0 ) {
				...
				if($var == 1){
					$entry_questionid = $this->Model_entryquestion->insertQuestion($entry_questiontitle,$entry_groupid);
	...
	public function doEdit($id){
		...
		if (! count($pesan)==0 ) {
			...
			if($var == 1){
				$update = $this->Model_entryquestion->updateQuestion($id,$entry_questiontitle,$entry_groupid);
