...
class Mock_exam_result extends CI_Controller {

	public $arrMenu = array();
	public $data;
	public $privilege = array();
	public $section = 12; //get module group id from database
	public $module_id = 34; //get module id from database
	public $module = "Mock Exam Result";
	...
	public function __construct(){
		...
		$this->load->model(array('backend/Model_mockexamresult','backend/Model_logcms'));
	...
	function view(){
		...
		$orderBy = " GROUP BY a.mock_exam_group_id, a.student_id ORDER BY a.mock_exam_result_id DESC";

		$cond 				= $where." ".$orderBy;
		$rsentry_question	= $this->Model_mockexamresult->getListResult($cond);
		$base_url			= BASE_URL_BACKEND."/mock_exam_result/view/";
		...
		$query = $this->Model_mockexamresult->getListResult($cond);
		
		if(count($query) > 0){
			for($i=0; $i<count($query); $i++){
				$rsMultipleChoice	= $this->Model_mockexamresult->getListResultDetail($query[$i]['mock_exam_group_id'], $query[$i]['student_id']);
				...
				$rsEssay	= $this->Model_mockexamresult->getListResultEssayDetail($query[$i]['mock_exam_group_id'], $query[$i]['student_id']);
				if(count($rsEssay) > 0){
		...
		$this->load->view('backend/mockexam_result/list');
	}


	public function editEssay($id){
		if (empty($id)) {
			redirect(BASE_URL_BACKEND."/mock_exam_result");
			exit();
		}
		...
		$rsResultEssay = $this->Model_mockexamresult->getListResultEssayDetailByID($id);  // mengambil database dari model untuk dikirim ke view
		...
		$this->load->view('backend/header',$this->data);
		$this->load->view('backend/mockexam_result/edit',$this->data);
	}
	...
	public function doEditEssay($id){
		...
		$mock_exam_essay_value = $this->security->xss_clean(secure_input($_POST['mock_exam_essay_value']));
		...
		// Validasi data
		if ($mock_exam_essay_value=="" || $mock_exam_essay_value=="0") {
		...
		if (! count($pesan)==0 ) {
			...
			$update = $this->Model_mockexamresult->updateResultEssay($id,$mock_exam_essay_value);
