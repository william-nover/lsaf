...
class Progress_exam_result extends CI_Controller {
	...
	public $module = "Progress Exam Result";
	...
	public function __construct(){
		...
		$this->load->model(array('backend/Model_progressexamresult','backend/Model_logcms'));
	...
	function view(){
		...
		$orderBy = " GROUP BY a.progress_exam_group_id, a.student_id ORDER BY a.progress_exam_result_id DESC";

		$cond 				= $where." ".$orderBy;
		$rsentry_question	= $this->Model_progressexamresult->getListResult($cond);
		$base_url			= BASE_URL_BACKEND."/progress_exam_result/view/";
		...
		$query = $this->Model_progressexamresult->getListResult($cond);
		
		if(count($query) > 0){
			for($i=0; $i<count($query); $i++){
				$rsMultipleChoice	= $this->Model_progressexamresult->getListResultDetail($query[$i]['progress_exam_group_id'], $query[$i]['student_id']);
				...
				$rsEssay	= $this->Model_progressexamresult->getListResultEssayDetail($query[$i]['progress_exam_group_id'], $query[$i]['student_id']);
				if(count($rsEssay) > 0){
		...
		$this->load->view('backend/progressexam_result/list');
	}


	public function editEssay($id){
		if (empty($id)) {
			redirect(BASE_URL_BACKEND."/progress_exam_result");
			exit();
		}
		...
		$rsResultEssay = $this->Model_progressexamresult->getListResultEssayDetailByID($id);  // mengambil database dari model untuk dikirim ke view
		...
		$this->load->view('backend/header',$this->data);
		$this->load->view('backend/progressexam_result/edit',$this->data);
	}
	...
	public function doEditEssay($id){
		...
		$progress_exam_essay_value = $this->security->xss_clean(secure_input($_POST['progress_exam_essay_value']));
		...
		// Validasi data
		if ($progress_exam_essay_value=="" || $progress_exam_essay_value=="0") {
		...
		if (! count($pesan)==0 ) {
			...
			$update = $this->Model_progressexamresult->updateResultEssay($id,$progress_exam_essay_value);
