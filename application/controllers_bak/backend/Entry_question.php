...
class Entry_question extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->load->model(array('backend/Model_entryquestion','backend/Model_logcms'));
	...
	/* public function loadDataGrid(){
		...
		$sidx = isset($_POST['sidx'])? $this->security->xss_clean(secure_input($_POST['sidx'])):'entry_question_name'; // get index row - i.e. user click to sort
        ...
        $count = $this->Model_entryquestion->getCountAllGrid($where);

		if( $count > 0 ) {

			$total_entry_question = ceil($count/$limit);    //calculating total number of entry_question

		} else {

			$total_entry_question = 0;

		}



		if ($page > $total_entry_question) $page=$total_entry_question;

   		$query = $this->Model_entryquestion->getAllGrid($start,$limit,$sidx,$sord,$where);

		

		$responce->page = $page;

		$responce->total = $total_entry_question;
		...
		if (count($query) > 0) {
			...
			foreach($query as $row) {
				...
				$responce->rows[$i]['id']=$row->entry_question_id;

				$row->entry_question_images = '<a id="viewBackend" href="#viewDataImage'.$row->entry_question_id.'"><img src="'.BASE_URL.str_replace('/admin/images','/admin/.thumbs/images',$row->entry_question_images).'" ></a><div style="display: none;"><div id="viewDataImage'.$row->entry_question_id.'"><img src="'.BASE_URL.$row->entry_question_images.'" ></div>';

				$responce->rows[$i]['cell']=array($row->entry_question_id,$row->entry_question_name, $row->entry_question_type, $row->entry_question_images, $row->entry_question_url, $row->entry_question_create_date,$row->entry_question_active_status);
	...
	function viewjqgrid(){
		...
		$this->data['title_grid'] = "Entry Question List";

		$this->data['caption'] = 'Question';
		...
		$this->load->view('backend/entry_question/list_jqgrid');
	...
	public function viewdatatables(){
		...
		$orderBy = "ORDER BY entry_question_id DESC";

		

		$cond 				= $where." ".$orderBy;

		$rsentry_question			= $this->Model_entryquestion->getListentry_question($cond);

		$this->data["Listentry_question"] = $rsentry_question;

		

		$this->load->view('backend/header',$this->data);

		$this->load->view('backend/entry_question/list_datatables');
	...
	function view(){
		...
		$orderBy = "ORDER BY entry_question_order ASC";
		...
		if (! count($pesan)==0 ) {
...

			{
...

				{
					...
					$entry_questionid = $this->Model_entryquestion->insertQuestion($entry_questiontitle);
					...
					$this->data['error']='Succes input question & answer';
	...
	public function doEdit($id){
		...
		if (! count($pesan)==0 ) {
...

			{
				...
				$update = $this->Model_entryquestion->updateQuestion($id,$entry_questiontitle);
