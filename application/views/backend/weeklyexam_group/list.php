              ...
              <div class="row">
                  <div class="col-lg-12">
                      <section class="panel">
						  ...
						  <form class="form-inline" role="form" method="post" name="frmSearch" action="<?php echo BASE_URL_BACKEND."/weekly_exam_group/view"; ?>">
						  <div class="panel-body">
							 <div class="col-lg-10">
								  <div class="form-group">
									  <select name="searchby" id="search-by" size="1" class="form-control">
										<option value="">Choose a search</option>
										<option value="weekly_exam_group_title" <?php if($searchby == "weekly_exam_group_title") { echo "selected"; }?> >Group Title</option>
                              ...
                              <section id="unseen">
								...
								<form name="formAssignment" method="POST" action="" onsubmit="return false;">
								  ...
								  <tr>
									  ...
									  <td><?php echo $data['weekly_exam_group_timer'];?></td>
									  <td><?php echo $data['weekly_exam_group_start'];?> - <?php echo $data['weekly_exam_group_end'];?></td>
									  <td><?php echo $data['weekly_exam_group_create_date'];?></td>
									  <td>
										<?php if($approve){ ?>
											<?php if($data['weekly_exam_group_active_status'] == 1) {?>
												<a class="btn-success btn-sm" title="Click to Inactive" href="<?php echo BASE_URL_BACKEND."/weekly_exam_group/active/".$data['weekly_exam_group_id'];?>"><i class="icon-ok"></i></a> &nbsp; 
											<?php } else { ?>
												<a class="btn-danger btn-sm" title="Click to Active" href="<?php echo BASE_URL_BACKEND."/weekly_exam_group/active/".$data['weekly_exam_group_id'];?>"><i class="icon-remove"></i></a> &nbsp;
											<?php } ?>
										<?php } ?>
										
										<?php if($edit){ ?>
											<a class="btn-primary btn-sm" title="Click to Edit" href="<?php echo BASE_URL_BACKEND;?>/weekly_exam_group/edit/<?php echo $data['weekly_exam_group_id'];?>"><i class="icon-pencil"></i></a> &nbsp; 
										<?php } ?>
										
										<?php if($delete){ ?>
											<a class="btn-danger btn-sm" title="Click to Delete" onclick="var answer = confirm('delete <?php echo $data['weekly_exam_group_title'];?> ?'); if (answer){window.location = '<?php echo BASE_URL_BACKEND;?>/weekly_exam_group/delete/<?php echo $data['weekly_exam_group_id'];?>';}"><span class="icon-trash"></span></a> &nbsp;
										<?php } ?>
									  </td>
								  </tr>
								  <?php } ?>
