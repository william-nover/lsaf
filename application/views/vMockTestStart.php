...
<title>Mock Exam - <?php echo $subject_title; ?></title>
	...
	$(function () {
		function expiredTimer(){
			...
			$.ajax({
				...
				dataType: "json",
				data: params,
				url: "<?php echo BASE_URL;?>/Mock_test_ajax/Timeout",
				success: function(data){
					if(data.status == 1){
						window.location.href = "<?php echo BASE_URL;?>/Mock_test/Finish"; 
					} else {
						alert(data.msg);
					}
				},
			...
			$.ajax({
				...
				dataType: "json",
				data: params,
				url: "<?php echo BASE_URL;?>/Mock_test_ajax/TimeoutEssay",
				success: function(data){
					if(data.status == 1){
						window.location.href = "<?php echo BASE_URL;?>/Mock_test/Finish"; 
					} else {
			...
			<?php if(count($AllQuestion) > 0) {?>
				<?php foreach($AllQuestion as $keyQstNo => $qstNo){ ?>
					<?php if($qstNo['is_answered'] == 1) {?>
						<a href="<?php echo BASE_URL;?>/Mock_test/Start/<?php echo $groupid; ?>/<?php echo $type; ?>/<?php echo $qstNo['no']; ?>"><div id="info-bullet"> <div> <?php echo $qstNo['no']; ?> </div> </div></a>
					<?php } else { ?>
						<a href="<?php echo BASE_URL;?>/Mock_test/Start/<?php echo $groupid; ?>/<?php echo $type; ?>/<?php echo $qstNo['no']; ?>"><div id="info-bullet-clear"> <div> <?php echo $qstNo['no']; ?> </div> </div></a>
					<?php } ?>
				<?php } ?>
			<?php } ?>
        </div>
...
<div class="question">
    ...
    <input type="hidden" name="question_id" id="idQuestion<?php echo $keyQuestion;?>" value="<?php echo $examQuestion['mock_exam_question_id'];?>">
	
	<?php if(!empty($examQuestion['mock_exam_question_images'])){ ?>
	<a id="questionImage" target="_blank" href="<?php echo $examQuestion['mock_exam_question_images'];?>"><img src="<?php echo $examQuestion['mock_exam_question_images'];?>" title="Question <?php echo $page;?>" height="425" class="gmbr-question" ></a>
	<?php } ?>
	<?php echo $examQuestion['mock_exam_question_title'];?>
	...
	<div class="pilihan">
		...
		<div>
            <input type="radio" name="answer_id<?php echo $keyQuestion;?>" id="idAnswer<?php echo $examQuestion['mock_exam_question_id'];?>" value="<?php echo $examAnswer['mock_exam_answer_id'];?>" <?php if($examAnswer['is_answered'] == 1) { echo "checked";} ?> />
            <label for="c1"><?php echo $examAnswer['mock_exam_answer_title'];?></label>
        </div>
		<?php } ?>
		<script type="text/javascript" >
			...
			$("input[name=answer_id<?php echo $keyQuestion;?>]:radio").change(function(e){
				...
				if(answerid != undefined){
					$.ajax({
						type : 'POST',
						dataType: "json",
						url: "<?php echo BASE_URL;?>/Mock_test_ajax/SubmitRadio",
						data: params,
						success: function(data){
...
<script type="text/javascript" >
	$(document).ready(function() {
		$('#btnFinish').click(function (e) {
			...
			var txt;
			var r = confirm("Are you sure want to finish Mock Exam ? ");
			if (r == true) {
				$.ajax({
					type : 'POST',
					dataType: "json",
					url: "<?php echo BASE_URL;?>/Mock_test_ajax/SubmitFinish",
					data: params,
					success: function(data){
						if(data.status == 1){
							window.location.href = "<?php echo BASE_URL;?>/Mock_test/Finish"; 
...
<div class="footer-2">
	<div class="footer-2-isi">
		...
		<?php if($pageNext <= $TotalQuestion){ ?>
			<a href="<?php echo BASE_URL;?>/Mock_test/Start/<?php echo $groupid; ?>/<?php echo $type; ?>/<?php echo $pageNext; ?>" class="next_question" id="btnNext"> <div class="next"> <div> Next </div> </div> </a>
		<?php } else { ?>
...
<div class="question-essay">
	<form id="myForm" action="<?php echo BASE_URL;?>/Mock_test_ajax/uploadEssay" method="post" enctype="multipart/form-data">
	<h1> Question </h1>
	...
	<?php echo "Number. ".$no;?>
	<?php if(!empty($examQuestion['mock_exam_question_images'])){ ?>
	<a id="questionImage" href="<?php echo BASE_URL.$examQuestion['mock_exam_question_images'];?>"><img src="<?php echo BASE_URL.$examQuestion['mock_exam_question_images'];?>" title="Question <?php echo $page;?>" height="125" class="gmbr-question-essay" align="right"></a>
	<?php } ?>
	<?php echo $examQuestion['mock_exam_question_title'];?>
    <?php } ?>
