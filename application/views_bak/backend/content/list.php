<body>
									  ...
									  <td>
										<?php if($approve){ ?>
											<?php if($content['content_active_status'] == 1) {?>
												<a class="btn-success btn-sm" title="Click to Inactive" href="<?php echo BASE_URL_BACKEND."/content/active/".$content['content_id'];?>"><i class="icon-ok"></i></a> &nbsp; 
											<?php } else { ?>
												<a class="btn-danger btn-sm" title="Click to Active" href="<?php echo BASE_URL_BACKEND."/content/active/".$content['content_id'];?>"><i class="icon-remove"></i></a> &nbsp;
											<?php } ?>
										<?php } ?>
										
										<?php if($edit){ ?>
											<a class="btn-primary btn-sm" title="Click to Edit" href="<?php echo BASE_URL_BACKEND;?>/content/edit/<?php echo $content['content_id'];?>"><i class="icon-pencil"></i></a> &nbsp; 
										<?php } ?>
										
										<?php if($delete){ ?>
											<a class="btn-danger btn-sm" title="Click to Delete" onclick="var answer = confirm('delete <?php echo $content['content_title'];?> ?'); if (answer){window.location = '<?php echo BASE_URL_BACKEND;?>/content/delete/<?php echo $content['content_id'];?>';}"><span class="icon-trash"></span></a> &nbsp;
										<?php } ?>
									  </td>
								  </tr>
								  <?php } ?>
	...
	function updateOrder(){
		...
		if(answer){
			frm.action = '<?php echo BASE_URL_BACKEND;?>/content/doOrder';
			frm.submit();
		} else {
			return false;
		}
	...
	$(document).ready(function() {
		...
		$("#langid\\[\\]").change(function(e){
			...
			if(size == 3){
				window.location = '<?php echo BASE_URL_BACKEND;?>/content/editLang/'+val;
			} else if(size == 2){
				window.location = '<?php echo BASE_URL_BACKEND;?>/content/addLang/'+val;
			}
		});
