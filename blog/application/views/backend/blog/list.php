                             ...
                             <table class="m-datatable" id="html_table" width="100%">
                                 ...
                                 <tbody>
                                    ...
                                    <tr>
                                       ...
                                       <?php if($order){ ?>
                                            <td align="center"><input type="text" class="form-control" style="text-align:center;" name="order[<?php echo $blog['blog_id'];?>]" size="1" maxlength="2" value="<?php echo $blog['blog_order'];?>"></td>
                                            <?php } ?>
                                       <td>
                                          <?php if($approve){ ?>
                                          <?php if($blog['blog_active_status'] == 1) {?>
                                          <a class="btn-success btn-sm" title="Click to Inactive" href="<?php echo BASE_URL_BACKEND."/".$controller."/active/".$blog['blog_id'];?>"><i class="la la-check-circle"></i></a> &nbsp; 
                                          <?php } else { ?>
                                          <a class="btn-danger btn-sm" title="Click to Active" href="<?php echo BASE_URL_BACKEND."/".$controller."/active/".$blog['blog_id'];?>"><i class="la la-power-off"></i></a> &nbsp;
                                          <?php } ?>
                                          <?php } ?>
                                          <?php if($edit){ ?>
                                          <a class="btn-primary btn-sm" title="Click to Edit" href="<?php echo BASE_URL_BACKEND.'/'.$controller;?>/edit/<?php echo $blog['blog_id'];?>"><i class="la la-edit"></i></a> &nbsp; 
                                          <?php } ?>
                                          <?php if($delete){ ?>
                                          <a class="btn-danger btn-sm" title="Click to Delete" onclick="var answer = confirm('delete <?php echo $blog['blog_title'];?> ?'); if (answer){window.location = '<?php echo BASE_URL_BACKEND.'/'.$controller;?>/delete/<?php echo $blog['blog_id'];?>';}"><span class="la la-trash"></span></a> &nbsp;
                                          <?php } ?>
                                       </td>
