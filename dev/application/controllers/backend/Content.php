...
//	public $alias_category = "aboutlsaf";
	...
	public function __construct()
	{
		...
		$this->load->model(array('backend/Model_menu_frontend','backend/Model_content','backend/Model_accordion','backend/Model_alias', 'backend/Model_language','backend/Model_logcms'));
	...
	function view(){
		...
		$cond 			= $where." ".$orderBy;
		$rsUserLevel	= $this->Model_content->getListContent($this->data['module_id'],$cond);
		$base_url		= BASE_URL_BACKEND.'/'.$this->data['controller']."/view/";
		...
		$ListContent = $this->Model_content->getListContent($this->data['module_id'],$cond);
		...
		if (! count($pesan)==0 ) {
			...
			//$countContent = count($cekContent);
				...
				if ($countAlias > 0 ) {
					...
					if(!empty($content_id)) {
						$aliasid = $this->Model_alias->insertAlias($content_id,$this->alias_category,$alias,"aboutlsaf/detail/".$content_id);
					}
	...
	function delete($id){
		...
		$delete = $this->Model_content->deleteContent($id);
                $this->Model_accordion->deleteAccordionby($id);
