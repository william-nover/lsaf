...
class Entry_result extends CI_Controller {
	...
	public function __construct(){
		...
		$this->load->model(array('backend/Model_entryresult','backend/Model_logcms'));
	...
	function view(){
		...
		$orderBy = "ORDER BY a.entry_result_id DESC";

		$cond 				= $where." ".$orderBy;
		$rsentry_question	= $this->Model_entryresult->getListResult($cond);
		$base_url			= BASE_URL_BACKEND."/entry_result/view/";
		...
		$query = $this->Model_entryresult->getListResult($cond);
		...
		$this->load->view('backend/entry_result/list');
	}
	...
	public function add(){
		...
		$this->load->view('backend/header',$this->data);
		$this->load->view('backend/entry_result/add',$this->data);
	}
	...
	public function doAdd(){
		...
		if (! count($pesan)==0 ) {
			...
			$cekGroup = $this->Model_entryresult->checkGroup($entry_grouptitle);
			...
			if ($countGroup > 0 ) {
				...
				$entry_groupid = $this->Model_entryresult->insertGroup($entry_grouptitle,$entry_grouptimer);
	...
	function delete($id){
		...
		$delete = $this->Model_entryresult->deleteGroup($id);
	...
	public function edit($id){
		...
		$rsGroup = $this->Model_entryresult->getGroup($id);  // mengambil database dari model untuk dikirim ke view
		...
		$this->load->view('backend/header',$this->data);
		$this->load->view('backend/entry_result/edit',$this->data);
	}
	...
	public function doEdit($id){
		...
		if (! count($pesan)==0 ) {
			...
			if ($countGroup > 0 ) {
				...
				$update = $this->Model_entryresult->updateGroup($id,$entry_grouptitle,$entry_grouptimer);
