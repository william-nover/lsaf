...
class Students extends CI_Controller {
	...
	public $alias_category = "students";
...
//                die;
	...
	function view(){
		...
		$base_url		= BASE_URL_BACKEND."/students/view/";
	...
	public function doAdd(){
		...
		$tb = $_POST['tbSave'];
		if (!$tb) {
			redirect(BASE_URL_BACKEND."/students");
			exit();
		}		
		$admin_data = $_SESSION['admin_data'];
		$this->data['admin_data'] = $admin_data;
		...
		if (! count($pesan)==0 ) {
			...
			if ($countContent > 0 ) {
				...
				if ($countAlias > 0 ) {
					...
					if(!empty($content_id)) {
						$aliasid = $this->Model_alias->insertAlias($content_id,$this->alias_category,$alias,"students/detail/".$content_id);
					}
