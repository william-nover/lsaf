...
class Model_banner extends CI_Model {
	...
	function getListBanner($cond = null){
		$query		= "SELECT banner_id, banner_name, banner_active_status, banner_images, banner_type, banner_url,banner_desc,
					  DATE_FORMAT( banner_create_date, '%d-%m-%Y %H:%i:%s' ) as banner_create_date, banner_create_by
	...
	function getBanner($id = '')
	{
		...
		$sql	= "SELECT banner_id, banner_name, banner_images, banner_type, banner_url, banner_desc, banner_active_status FROM tbl_banner $where ORDER BY banner_id ASC";	
	...
	function updateBanner($id,$bannername,$bannersimageurl,$bannertype,$bannerurl,$bannerdesc)
	{
