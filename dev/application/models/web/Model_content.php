...
class Model_content extends CI_Model {
	...
	{
		 $data = array();
			 $sql = "SELECT a.accordion_id,a.accordion_title,a.accordion_desc from tbl_accordion as a  where a.accordion_active_status=1 and  a.content_id= '".$content_id."' order by accordion_order asc";
			$query = $this->db->query($sql);
			if($query->num_rows() > 0){
