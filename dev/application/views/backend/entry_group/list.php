							 ...
							 <div class="col-lg-10">
								  <div class="form-group">
									  <select name="searchby" id="search-by" size="1" class="form-control">
										...
										<option value="entry_group_title" <?php if($searchby == "entry_question_title") { echo "selected"; }?> >Question Title</option>
                              ...
                              <section id="unseen">
								...
								<form name="formAssignment" method="POST" action="" onsubmit="return false;">
								  ...
								  <tr>
									  ...
									  <td><?php echo $data['entry_group_timer'];?></td>
									  <td><?php echo $data['entry_group_create_date'];?></td>
									  <td>
										<?php if($approve){ ?>
											<?php if($data['entry_group_active_status'] == 1) {?>
												<a class="btn-success btn-sm" title="Click to Inactive" href="<?php echo BASE_URL_BACKEND."/entry_group/active/".$data['entry_group_id'];?>"><i class="icon-ok"></i></a> &nbsp; 
											<?php } else { ?>
												<a class="btn-danger btn-sm" title="Click to Active" href="<?php echo BASE_URL_BACKEND."/entry_group/active/".$data['entry_group_id'];?>"><i class="icon-remove"></i></a> &nbsp;
											<?php } ?>
										<?php } ?>
										
										<?php if($edit){ ?>
											<a class="btn-primary btn-sm" title="Click to Edit" href="<?php echo BASE_URL_BACKEND;?>/entry_group/edit/<?php echo $data['entry_group_id'];?>"><i class="icon-pencil"></i></a> &nbsp; 
										<?php } ?>
										
										<?php if($delete){ ?>
											<a class="btn-danger btn-sm" title="Click to Delete" onclick="var answer = confirm('delete <?php echo $data['entry_group_title'];?> ?'); if (answer){window.location = '<?php echo BASE_URL_BACKEND;?>/entry_group/delete/<?php echo $data['entry_group_id'];?>';}"><span class="icon-trash"></span></a> &nbsp;
										<?php } ?>
									  </td>
								  </tr>
								  <?php } ?>
